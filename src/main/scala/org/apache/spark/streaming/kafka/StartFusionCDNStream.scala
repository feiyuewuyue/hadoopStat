...
//  val CHECKPOINT_DIR: String = BasicSettingReader.configration.get("checkpint.dir.fusion").toString()
  ...
  val kafkaGroupids = BasicSettingReader.configration.get("kafka.groupid.fcdn.statis").toString()
  ...
  def functionToCreateContext(): StreamingContext = {
   ...
   // sparkConf.set("spark.speculation", "false")
    ...
    val lines = message.transform { rdd =>
      ...
      //??为什么这这里取offset和将offsetRanges传给parseFusionKafkaBytesWithOffset 后计算不一样呢
       val offset=StreamProcess.genPartitionMinOffset(offsetRanges)
      val rddnew=rdd.flatMap(f => {
      val hmDetail=  StreamProcess.parseFusionKafkaBytesForBand(f._2,offset)
        val res = StreamProcess.transDetailLogToFusionBand(hmDetail)
      res
      })
      rddnew
//        rdd.map(f => StreamProcess.parseFusionKafkaBytesWithOffset(f._2,offset))
    }
...
//   val rddK = lines.mapPartitions(f => { StreamProcess.mergeDetailLogFusionForBand(f) }, true)
    ...
    rddRes.foreachRDD(rddxm => {
      if (!rddxm.isEmpty()) {
        ...
        rddSave.saveToEs("{index_name}/{index_type}", Map("es.mapping.id" -> "id", "es.mapping.exclude" -> "id,index_name,index_type,offset"))//offset
      }
