...
object onlyOnceProcess1 {
  ...
  kafkaStream.transform{rdd=>
     ...
     // offsetRanges = rdd.asInstanceOf[HasOffsetRanges].offsetRanges
      ...
      val logDataFrame = rdd.map(attributes=>RongheData(attributes(0).toString(),attributes(1).toString(),attributes(2).toString(),attributes(3).toString(),attributes(4).toString(),attributes(5).toString(),
          ...
          attributes(33).toString(),attributes(34).toString(),attributes(35).toString(),attributes(36).toString(),DigestUtils.md5Hex(attributes(35)+"-"+attributes(36)),attributes(35).split("_")(2).substring(0, 8),attributes(0).toLowerCase(),"","")).toDF().saveToEs(args(11)+"{firm_name}-{filetime}/"+args(12),Map("es.mapping.id" ->"id","es.mapping.exclude" -> "id,filetime"))
      //注册为tempTable
