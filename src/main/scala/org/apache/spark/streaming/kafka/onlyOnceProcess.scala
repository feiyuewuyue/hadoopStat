...
object onlyOnceProcess {
  ...
  //  val kafkaParams=Map[String,String]("metadata.broker.list" -> brokerList,
   //     "zookeeper.connect"->zkHost,"group.id"->(args(7)+"-1"),"auto.offset.reset"->"smallest","auto.commit.enable"->"false") //largest,smallest
  ...
  kafkaStream.transform{rdd=>
    ...
    if (!rdd.isEmpty()) {
      ...
      val logDataFrame = rdd.map(attributes=>RongheData(attributes(0).toString(),attributes(1).toString(),attributes(2).toString(),attributes(3).toString(),attributes(4).toString(),attributes(5).toString(),
          ...
          attributes(33).toString(),attributes(34).toString(),attributes(35).toString(),attributes(36).toString(),"","",attributes(0).toString(),"","")).toDF()
      //注册为tempTable
