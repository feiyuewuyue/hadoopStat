...
/*  val CHECKPOINT_DIR: String = BasicSettingReader.configration.get("checkpint.dir.fusion").toString()
  ...
  val esPort= BasicSettingReader.configration.get("es.port.fusion").toString()*/
   ...
   val CHECKPOINT_DIR: String ="hdfs://nameservice1/zrk_test/zrk_checkpoint_fusion/"
  ...
  def main(args1: Array[String]) {
   ...
   //System.setProperty("hadoop.home.dir", "D:\\hadoop");
     ...
     val batch_interval =args1(0)
     val kafkahosts ="183.131.54.162:9092,183.131.54.163:9092,183.131.54.164:9092,183.131.54.165:9092,183.131.54.166:9092"
     val kafkaTopics=args1(1)
     
      val kafkaGroupids = args1(2)
  val esNodes= "183.136.128.47"
  val esPort= "9200"
     val maxRatePerPartition=args1(3)
     
      val sparkConf = new SparkConf().setAppName("fusionDetailtest")
   ...
   /* sparkConf.set("spark.serializer", "org.apache.spark.serializer.KryoSerializer")
    ...
    sparkConf.set("es.nodes", "183.136.128.47") //183.136.128.47:9200 183.131.54.181
...
*/
    ...
    sparkConf.set("spark.task.maxFailures","1")
    sparkConf.set("spark.speculation","false")
    sparkConf.set("spark.streaming.kafka.maxRatePerPartition",args1(3))
   ...
   // sparkConf.set("es.batch.size.bytes","10m")
    ...
    sparkConf.set("spark.driver.userClassPathFirst", "true");
    sparkConf.set("es.index.auto.create", "true")
    sparkConf.set("es.batch.size.bytes", "350mb")
    sparkConf.set("es.batch.size.entries", "15000")
    sparkConf.set("es.batch.write.refresh", "false")
    ...
    message.foreachRDD(rddstr => {
      if (!rddstr.isEmpty()) {
        ...
        if (rddK != null && rddK.partitions.length > 0) {
          rddK.saveToEs(args1(4)+"/zrktest_type", Map("es.mapping.id" -> "id","es.mapping.exclude" -> "index_name,index_type"))  //, Map("es.mapping.id" -> "id","es.mapping.exclude" -> "index_name,index_type")
  ...
  //  val ssc = functionToCreateContext(kafkahosts,kafkaTopics,kafkaGroupids,batch_interval,maxRatePerPartition)//StreamingContext.getOrCreate(CHECKPOINT_DIR, functionToCreateContext)

    ssc.start()
    ssc.awaitTermination()
  }
  def functionToCreateContext(kafkahosts:String,kafkaTopics:String,kafkaGroupids:String,batch_interval:String,maxRatePerPartition:String): StreamingContext = {
...
/*    message.foreachRDD(rddstr => {
      if (!rddstr.isEmpty()) {
        ...
        val rddK = rddline.mapPartitions(f => { StreamProcess.getFusionDetailLog(f) }, true)
        if (rddK != null && rddK.partitions.length > 0) {
          rddK.saveToEs("test-tx-zrk/zrktest_type", Map("es.mapping.id" -> "id","es.mapping.exclude" -> "index_name,index_type"))
          km.updateZKOffsets(offsetRanges)
