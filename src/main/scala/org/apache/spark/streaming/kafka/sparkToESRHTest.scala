      ...
      kafkaStream.foreachRDD(rdds=>{
       val sqlContext = SQLContextSingleton.getInstance(rdds.sparkContext)
        import sqlContext.implicits._
         ...
         rdds.map(x=>converToRonghe(x._2, args(8))).toDF().saveToEs(args(11)+"{lowfirmname}-{filetime}/"+args(12),Map("es.mapping.id" ->"id","es.mapping.exclude" -> "id,filetime,lowfirmname"))
